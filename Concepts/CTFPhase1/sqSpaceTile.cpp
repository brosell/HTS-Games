...
fog_nowVisible(false)
{
  ...
  if (s_baseSkin == 0)
  {
    ...
    s_baseSkin->addImage("images\\Square\\Tile_Empty.bmp", 1);
    s_selectedSkin = new RotateSkin(1);
    s_selectedSkin->addImage("images\\Square\\Tile_Selected.bmp", 1);

    s_tileSolid = new RotateSkin(1);
    s_tileSolid->addImage("images\\Square\\tile_solid.bmp", 1);

    s_solidFog = new RotateSkin(1);
    s_solidFog->addImage("images\\Square\\Fog_Solid.bmp", 1);

    s_partialFog = new RotateSkin(1);
    s_partialFog->addImage("images\\Square\\Fog_Partial.bmp", 1);
  }
