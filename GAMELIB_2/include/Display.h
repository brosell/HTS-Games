...
public:
  ...
  virtual void stretchBlit(HTexture textureId, Rect& dest) = 0;
  ...
  SDL_Surface* createSDLSurface(int w, int h, SDL_Surface* inspiration = 0);
  ...
  HTexture prepare(SDL_Surface* surface, string name="");
...
inline SDL_Surface* createSDLSurface(int w, int h, SDL_Surface* inspiration = 0)
{
  return Display::getInstance()->createSDLSurface(w, h, inspiration);
}

inline Display* displayInstance() { return Display::getInstance(); }

class ReleaseTexture: public std::unary_function<HTexture, void>
{
public:
  operator()(HTexture tid)
  {
    Display::getInstance()->release(tid);
  }
};
