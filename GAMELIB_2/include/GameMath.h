...
{
public:
  PointDist(): point(0, 0), dist(0) { }
  PointDist(Point p, float d):point(p), dist(d) { }
  PointDist(int x, int y, float d):point(x, y), dist(d) { }
  Point point;
  float dist;
};

PointDist closestPoint(float px, float py, float X1, float Y1, float X2, float Y2);
inline PointDist closestPoint(Point p, Line l) { return closestPoint(p.x, p.y, l.p1.x, l.p1.y, l.p2.x, l.p2.y); }
