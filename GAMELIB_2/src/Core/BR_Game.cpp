...
m_thinkTimer(0)
{
  ...
  m_thinkTimer = Timer::create("ThinkTimer", 1001, 1000/30, this, 0, false);
}
...
void BR_Game::timeReport()
{
  ...
  logError(Inform, "tickTime   : %03d", m_sw_ticktime);
  logError(Inform, "maxtickTime: %03d", m_sw_maxticktime);
  logError(Inform, "-----------------------");
...
void BR_Game::timeout(TimerEvent* event)
{
  if (event->id == 1001)
  {
    StopWatch sw;
    sw.start();
    ThinkScheduler::newFrame();
    sw.stop();
    m_thinkTimer = Timer::create("ThinkTimer", 1001, 1000/30-(sw.getElapsedTime()), this, 0, false);
    m_sw_maxticktime = hts::max(m_sw_ticktime, (int)sw.getElapsedTime());
    m_sw_ticktime = sw.getElapsedTime();
    return;
  }
