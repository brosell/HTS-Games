...
void HSlider::setSliderSkin(string assetName)
{
  ...
  m_slider = theGame.am->getSkin(assetName);
...
{
  if (m_hotSlider.isValid())
  {
    m_hotSlider->removeListener(this);
  }
  m_hotSlider = theGame.am->getSkin(assetName);
  if (m_hotSlider.isValid())
  {
    m_hotSlider->addListener(this);
  }
  setDirty(true);
}
...
void HSlider::paint(Display* surface)
{
  if (m_slider.isValid())
  {
    ...
    {
      sliderRelX = (((float)m_position/(float)(m_highBound - m_lowBound)) * (r.w - m_slider->getWidth()));
    }
