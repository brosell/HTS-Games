...
// A CommandLine object is created by feeding the classic
...
CommandLine::ParseSettings(const std::string& strInput) throw (std::string)
{
	...
	while ((strNextWord.length()) > 0 || (ist >> strWord))
	{
		...
		if (IsParamName(strWord))
		{
			...
			{
				pResult = mArgMap.insert(std::make_pair(GetParamName(strWord),
									 					std::string("")));
				if(!pResult.second)
				{
					throw(std::string(
			...
			else if ( strNextWord.length() > 0 )
			{
				...
				{
					throw(std::string(
						"Detected a duplicate command line parameter. (2)"));
				}
				++mnParameterCount;
				if(! (ist >> strNextWord))
				{	break;
						...
						throw (std::string)
{
	...
	throw (std::string("-") + strArgName + " is required, but was not found.");
}
